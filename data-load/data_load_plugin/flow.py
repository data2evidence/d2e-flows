            ...
            for i, data in read_csv(file.path, escapechar=escape_char, header=header, delimiter=options.delimiter, encoding=options.encoding, chunksize=chunksize):
                if(header):
                    ...
                    if (file.table_name == "concept_relationship" or file.table_name == "concept" or file.table_name == "drug_strength"):
                        ...
                        data["valid_start_date"] = pd.to_datetime(data["valid_start_date"], format='%d%m%Y', errors='coerce')
                        data["valid_end_date"] = pd.to_datetime(data["valid_end_date"], format='%d%m%Y', errors='coerce')
                    if(file.table_name == "drug_strength"):
                        data["amount_value"].replace("", "null")
                        data["amount_unit_concept_id"].replace("", "null")
                        data["box_size"].replace("", "null")
                        data["numerator_value"].replace("", "null")
                        data["numerator_unit_concept_id"].replace("", "null")
                        data["denominator_unit_concept_id"].replace("", "null")
