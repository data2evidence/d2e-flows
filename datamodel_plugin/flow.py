...
def create_ms_datamodel(options: createDataModelType):
    dbsvc_module = importlib.import_module('d2e_dbsvc')
    dbsvc_module.create_datamodel(options)


@flow(log_prints=True, task_runner=SequentialTaskRunner)
def update_ms_datamodel(options: updateDataModelType):
    dbsvc_module = importlib.import_module('d2e_dbsvc')
    dbsvc_module.update_datamodel(options)

    
@flow(log_prints=True, task_runner=SequentialTaskRunner)
def create_update_ms_datamodel(options: dataModelType):
    dbsvc_module = importlib.import_module('d2e_dbsvc')
