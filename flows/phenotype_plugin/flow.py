...
def setup_plugin():
    ...
    if (r_libs_user_directory):
        ...
        ShellOperation(
            commands=[
                # f"""GITHUB_PAT=$(Rscript -e "cat(Sys.getenv('GITHUB_PAT'))") """,
                # f"""Rscript -e "remotes::install_github('OHDSI/CohortGenerator@v0.8.1', quiet=FALSE, upgrade='never', auth_token='$GITHUB_PAT', force=TRUE, verbose=TRUE, dependencies=FALSE, lib='{r_libs_user_directory}')" """
