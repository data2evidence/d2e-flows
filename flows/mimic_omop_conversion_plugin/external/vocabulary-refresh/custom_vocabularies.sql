...
DROP TABLE IF EXISTS mimic_staging.tmp_custom_concept;

CREATE TABLE mimic_staging.tmp_custom_concept AS
SELECT voc.source_concept_id       AS concept_id,
...
FROM mimic_staging.tmp_custom_mapping voc
GROUP BY voc.source_concept_id,
         voc.concept_name,
...
DROP TABLE IF EXISTS mimic_staging.tmp_custom_concept_relationship;

CREATE TABLE mimic_staging.tmp_custom_concept_relationship AS
SELECT tcr.source_concept_id             AS concept_id_1,
       CASE
...
FROM mimic_staging.tmp_custom_mapping tcr
...
DROP TABLE IF EXISTS mimic_staging.tmp_custom_vocabulary_dist;

CREATE TABLE mimic_staging.tmp_custom_vocabulary_dist AS
SELECT voc.source_vocabulary_id AS source_vocabulary_id,
...
DROP TABLE IF EXISTS mimic_staging.tmp_custom_vocabulary;

CREATE TABLE mimic_staging.tmp_custom_vocabulary AS
SELECT voc.source_vocabulary_id    AS vocabulary_id,
       voc.source_vocabulary_id    AS vocabulary_name,
       'Odysseus generated'        AS vocabulary_reference,
       CAST(NULL AS varchar(1024)) AS vocabulary_version,
       2110000001 +
       ROW_NUMBER() OVER (
...
FROM mimic_staging.tmp_custom_vocabulary_dist voc
;
...
DROP TABLE IF EXISTS mimic_staging.tmp_voc_concept;

-- ADDED COMMENT: This copies over only the standard OMOP concepts (ID < 2e9)
CREATE TABLE mimic_staging.tmp_voc_concept AS
SELECT *
...
DROP TABLE IF EXISTS mimic_staging.tmp_voc_concept_relationship;

CREATE TABLE mimic_staging.tmp_voc_concept_relationship AS
SELECT vr.*
FROM mimic_staging.concept_relationship vr
         INNER JOIN
     mimic_staging.tmp_voc_concept vc1
     ON vc1.concept_id = vr.concept_id_1
         INNER JOIN
     mimic_staging.tmp_voc_concept vc2
...
INSERT INTO mimic_staging.tmp_voc_concept
SELECT voc.concept_id             AS concept_id,
       voc.concept_name           AS concept_name,
       ...
       voc.concept_code           AS concept_code,
       voc.valid_start_date::date AS valid_start_date,
       voc.valid_end_date::date   AS valid_end_date,
       voc.invalid_reason         AS invalid_reason,

       voc.load_table_id           AS load_table_id,
       voc.load_row_id              AS load_row_id
       -- when loaded not by vocabulary_refresh, raw concept tables have no load_row_id, so comment it temporarily
FROM mimic_staging.tmp_custom_concept voc
-- LEFT JOIN
--     mimic_staging.concept vc
...
SELECT *
FROM mimic_staging.tmp_voc_concept;

-- ----------------------------------------------------------------------
-- Add relationships to the added custom concepts
-- ----------------------------------------------------------------------

INSERT INTO mimic_staging.tmp_voc_concept_relationship
SELECT tcr.concept_id_1           AS concept_id_1,
       tcr.concept_id_2           AS concept_id_2,
       tcr.relationship_id        AS relationship_id,
       tcr.valid_start_date::date AS valid_start_date,
       tcr.valid_end_date::date   AS valid_end_date,
       tcr.invalid_reason         AS invalid_reason,

       tcr.load_table_id            AS load_table_id,
       tcr.load_row_id              AS load_row_id
FROM mimic_staging.tmp_custom_concept_relationship tcr
-- LEFT JOIN 
--     mimic_staging.concept_relationship vcr
...
SELECT *
FROM mimic_staging.tmp_voc_concept_relationship;

-- ----------------------------------------------------------------------
-- Re-write vocabularies to remove previous version of custom vocabularies
-- ----------------------------------------------------------------------

DROP TABLE IF EXISTS mimic_staging.tmp_voc_vocabulary;

-- ADDED COMMENT: This copies over only the standard OMOP concepts (ID < 2e9)
CREATE TABLE mimic_staging.tmp_voc_vocabulary AS
SELECT *
FROM mimic_staging.vocabulary
...
INSERT INTO mimic_staging.tmp_voc_vocabulary
SELECT voc.vocabulary_id         AS vocabulary_id,
       ...
       voc.load_table_id           AS load_table_id,
       voc.load_row_id             AS load_row_id
FROM mimic_staging.tmp_custom_vocabulary voc
;
...
SELECT *
FROM mimic_staging.tmp_voc_vocabulary;

INSERT INTO mimic_staging.concept
SELECT vcv.vocabulary_concept_id  AS concept_id,
       ...
       NULL                        AS load_row_id
FROM mimic_staging.tmp_custom_vocabulary vcv
;
...
DROP TABLE IF EXISTS mimic_staging.tmp_custom_concept_skipped;

CREATE TABLE mimic_staging.tmp_custom_concept_skipped AS
SELECT tcc.*
FROM mimic_staging.tmp_custom_concept tcc
         INNER JOIN
     mimic_staging.concept vc
