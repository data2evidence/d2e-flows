--relationships cycle
DROP TABLE IF EXISTS mimic_staging.z_check_voc_1;
CREATE TABLE mimic_staging.z_check_voc_1
AS
SELECT 1                        check_id,
       'relationships cycle' AS check_name,
       r.*
FROM mimic_staging.concept_relationship r,
     mimic_staging.concept_relationship r_int
...
DROP TABLE IF EXISTS mimic_staging.z_check_voc_2;
CREATE TABLE mimic_staging.z_check_voc_2
AS
SELECT 2                                                         check_id,
...
FROM mimic_staging.concept_relationship r,
     mimic_staging.concept_relationship r_int,
     mimic_staging.relationship rel
WHERE r.invalid_reason IS NULL
...
DROP TABLE IF EXISTS mimic_staging.z_check_voc_3;
CREATE TABLE mimic_staging.z_check_voc_3
AS
SELECT 3                                  check_id,
...
WHERE r.relationship_id = rel.relationship_id
  AND NOT EXISTS (SELECT 1
                  FROM mimic_staging.concept_relationship r_int
...
DROP TABLE IF EXISTS mimic_staging.z_check_voc_6;
CREATE TABLE mimic_staging.z_check_voc_6
AS
SELECT 6                                             check_id,
...
DROP TABLE IF EXISTS mimic_staging.z_check_voc_7;
CREATE TABLE mimic_staging.z_check_voc_7
AS
SELECT 7                                                                             check_id,
...
WHERE (
              ...
              OR (
                              c.valid_end_date = '20991231'::date
...
-- OR c.valid_start_date > DATE_ADD(COALESCE(vc.latest_update, CURRENT_DATE()), INTERVAL 15 YEAR) --some concepts might be FROM near future (e.g. GGR, HCPCS) [AVOF-1015]/increased 20180928 for some NDC concepts
              OR c.valid_start_date < '19000101'::date -- some concepts have a real date < 1970
          )
;

--wrong valid_start_date, valid_end_date or invalid_reason for the voc_concept_relationship
DROP TABLE IF EXISTS mimic_staging.z_check_voc_8;
CREATE TABLE mimic_staging.z_check_voc_8
AS
SELECT 8                                                                                              check_id,
...
FROM (SELECT r.*,
             CASE
                 WHEN (
                                  r.valid_end_date = '20991231'::date
                     ...
                     OR r.valid_start_date < '19700101'::date
                     THEN 1
                 ELSE 0
                 END check_flag
      FROM mimic_staging.concept_relationship r) AS s0
WHERE check_flag = 1
;

--RxE to Rx name duplications
DROP TABLE IF EXISTS mimic_staging.z_check_voc_9;
CREATE TABLE mimic_staging.z_check_voc_9
AS
SELECT 9                                check_id,
...
FROM mimic_staging.concept c1
         JOIN mimic_staging.concept c2 ON upper(c2.concept_name) = upper(c1.concept_name)
...
DROP TABLE IF EXISTS mimic_staging.z_check_voc_10;
CREATE TABLE mimic_staging.z_check_voc_10
AS
SELECT 10                                     check_id,
...
FROM mimic_staging.concept_relationship r
     ...
     (SELECT r_int.concept_id_1,
      ...
      GROUP BY r_int.concept_id_1,
               r_int.relationship_id
      HAVING COUNT(*) > 1) r_dup
     ON r.concept_id_1 = r_dup.concept_id_1
         AND r.relationship_id = r_dup.relationship_id
;

--wrong concept_name [AVOF-1438]
DROP TABLE IF EXISTS mimic_staging.z_check_voc_11;
CREATE TABLE mimic_staging.z_check_voc_11
AS
SELECT 11                                                                check_id,
...
DROP TABLE IF EXISTS mimic_staging.z_check_voc_12;
CREATE TABLE mimic_staging.z_check_voc_12
AS
SELECT 12                                                             check_id,
       'wrong relationships: "Maps to"/"Maps to value" not to "S"' AS check_name,
       r.*
FROM mimic_staging.concept c2,
     mimic_staging.concept_relationship r
...
DROP TABLE IF EXISTS mimic_staging.z_check_voc_13;
CREATE TABLE mimic_staging.z_check_voc_13
AS
SELECT 13                                                                                              check_id,
...
DROP TABLE IF EXISTS mimic_staging.z_check_voc_14;
CREATE TABLE mimic_staging.z_check_voc_14
AS
SELECT 14                             check_id,
       'nonexistent relationships' AS check_name,
       r.relationship_id
FROM (SELECT relationship_id
      FROM mimic_staging.concept_relationship
      EXCEPT
      DISTINCT
      SELECT relationship_id
      FROM mimic_staging.relationship) r;

DROP TABLE IF EXISTS mimic_staging.z_check_voc_15;

CREATE TABLE mimic_staging.z_check_voc_15
AS
SELECT 15                       check_id,
       'nonexistent classes' AS check_name,
       r.concept_class_id
FROM (SELECT concept_class_id
      FROM mimic_staging.concept
      EXCEPT
      DISTINCT
      SELECT concept_class_id
      FROM mimic_staging.concept_class) r;

DROP TABLE IF EXISTS mimic_staging.z_check_voc_16;

CREATE TABLE mimic_staging.z_check_voc_16
AS
SELECT 16                       check_id,
...
FROM (SELECT domain_id
      ...
      DISTINCT
      SELECT domain_id
      FROM mimic_staging.domain) r;

DROP TABLE IF EXISTS mimic_staging.z_check_voc_17;
CREATE TABLE mimic_staging.z_check_voc_17
AS
SELECT 17                            check_id,
...
FROM (SELECT vocabulary_id
      ...
      FROM mimic_staging.vocabulary) r;

-- create and show summary

DROP TABLE IF EXISTS mimic_staging.z_check_voc_errors_summary;
CREATE TABLE mimic_staging.z_check_voc_errors_summary
AS
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_1
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_2
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_3
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_6
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_7
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_8
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_9
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_10
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_11
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_12
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_13
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_14
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_15
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_16
GROUP BY check_id, check_name
UNION ALL
SELECT check_id, check_name, COUNT(*) AS row_count
FROM mimic_staging.z_check_voc_17
GROUP BY check_id, check_name
;

SELECT *
FROM mimic_staging.z_check_voc_errors_summary
ORDER BY check_id;
