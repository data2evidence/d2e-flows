...
DROP TABLE IF EXISTS mimic_etl.lk_micro_cross_ref;
CREATE TABLE mimic_etl.lk_micro_cross_ref AS
SELECT
...
FROM
    mimic_etl.src_microbiologyevents src -- mbe
;
...
DROP TABLE IF EXISTS mimic_etl.lk_micro_hadm_id;
CREATE TABLE mimic_etl.lk_micro_hadm_id AS
SELECT
...
FROM  
    mimic_etl.lk_micro_cross_ref src
INNER JOIN 
...
DROP TABLE IF EXISTS mimic_etl.lk_meas_organism_clean;
CREATE TABLE mimic_etl.lk_meas_organism_clean AS
SELECT DISTINCT
...
FROM
    mimic_etl.src_microbiologyevents src -- mbe
INNER JOIN
    mimic_etl.lk_micro_cross_ref cr
...
DROP TABLE IF EXISTS mimic_etl.lk_specimen_clean;
CREATE TABLE mimic_etl.lk_specimen_clean AS
SELECT DISTINCT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
...
FROM
    mimic_etl.lk_meas_organism_clean src -- mbe
INNER JOIN
    mimic_etl.lk_micro_cross_ref cr
...
DROP TABLE IF EXISTS mimic_etl.lk_meas_ab_clean;
CREATE TABLE mimic_etl.lk_meas_ab_clean AS
SELECT
    src.subject_id                              AS subject_id,
    src.hadm_id                                 AS hadm_id,
    cr.start_datetime                           AS start_datetime,
...
FROM
    mimic_etl.src_microbiologyevents src
INNER JOIN
    mimic_etl.lk_micro_cross_ref cr
...
DROP TABLE IF EXISTS mimic_etl.lk_d_micro_clean;
CREATE TABLE mimic_etl.lk_d_micro_clean AS
SELECT
    dm.itemid                                       AS itemid,
...
FROM
    mimic_etl.lk_meas_ab_clean src
WHERE
...
DROP TABLE IF EXISTS mimic_etl.lk_d_micro_concept;
CREATE TABLE mimic_etl.lk_d_micro_concept AS
SELECT
    dm.itemid                   AS itemid,
...
DROP TABLE IF EXISTS mimic_etl.lk_specimen_mapped;
CREATE TABLE mimic_etl.lk_specimen_mapped AS
SELECT nextval('id_sequence') AS specimen_id, t.*
FROM (SELECT
    src.subject_id                              AS subject_id,
...
FROM
    mimic_etl.lk_specimen_clean src
INNER JOIN
    mimic_etl.lk_d_micro_concept mc
        ON src.spec_itemid = mc.itemid
LEFT JOIN
    mimic_etl.lk_micro_hadm_id hadm
...
DROP TABLE IF EXISTS mimic_etl.lk_meas_organism_mapped;
CREATE TABLE mimic_etl.lk_meas_organism_mapped AS
...
FROM (SELECT
    ...
    src.trace_id                    AS trace_id
FROM
    mimic_etl.lk_meas_organism_clean src
INNER JOIN
    mimic_etl.lk_d_micro_concept tc
        ON src.test_itemid = tc.itemid
INNER JOIN
    mimic_etl.lk_d_micro_concept sc
        ON src.spec_itemid = sc.itemid
LEFT JOIN
    mimic_etl.lk_d_micro_concept oc
...
DROP TABLE IF EXISTS mimic_etl.lk_meas_ab_mapped;
CREATE TABLE mimic_etl.lk_meas_ab_mapped AS
SELECT nextval('id_sequence') AS measurement_id, t.*
FROM (SELECT
    src.subject_id                              AS subject_id,
    COALESCE(src.hadm_id, hadm.hadm_id)         AS hadm_id,
...
FROM
    mimic_etl.lk_meas_ab_clean src
INNER JOIN
    mimic_etl.lk_d_micro_concept ac
        ON src.ab_itemid = ac.itemid
LEFT JOIN
    mimic_etl.lk_d_micro_concept rc
        ON src.interpretation = rc.source_code
        AND rc.source_vocabulary_id = 'mimiciv_micro_resistance' -- new vocab
LEFT JOIN
    mimic_etl.lk_meas_operator_concept opc -- see lk_meas_labevents.sql
