...
DROP TABLE IF EXISTS mimic_etl.lk_meas_d_labitems_clean;
CREATE TABLE mimic_etl.lk_meas_d_labitems_clean AS
SELECT dlab.itemid                                                                            AS itemid,       -- for <cdm>.<source_value>
       ...
       (CASE WHEN dlab.loinc_code IS NOT NULL THEN 'LOINC' ELSE 'mimiciv_meas_lab_loinc' END) AS source_vocabulary_id
FROM mimic_etl.src_d_labitems dlab
;
...
DROP TABLE IF EXISTS mimic_etl.lk_meas_labevents_clean;
CREATE TABLE mimic_etl.lk_meas_labevents_clean AS
...
FROM (SELECT src.subject_id                                             AS subject_id,
             ...
             regexp_extract(src.value, '^(<=|>=|>|<|=|)', 0)            AS value_operator,
             NULLIF(regexp_extract(src.value, '[-]?[\d]+[.]?[\d]*', 0), '')        AS value_number,  -- assume "-0.34 etc"
      ...
      FROM mimic_etl.src_labevents src
               INNER JOIN
           mimic_etl.src_d_labitems dlab
           ON src.itemid = dlab.itemid) t
;
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS mimic_etl.lk_meas_d_labitems_concept;
CREATE TABLE mimic_etl.lk_meas_d_labitems_concept AS
...
FROM mimic_etl.lk_meas_d_labitems_clean dlab
         LEFT JOIN
...
DROP TABLE IF EXISTS mimic_etl.lk_meas_labevents_hadm_id;
CREATE TABLE mimic_etl.lk_meas_labevents_hadm_id AS
...
FROM mimic_etl.lk_meas_labevents_clean src
         INNER JOIN
...
DROP TABLE IF EXISTS mimic_etl.lk_meas_labevents_mapped;
CREATE TABLE mimic_etl.lk_meas_labevents_mapped AS
