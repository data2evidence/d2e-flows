...
DROP TABLE IF EXISTS mimic_etl.d_items_to_concept;
CREATE TABLE mimic_etl.d_items_to_concept AS
WITH
counts AS
(
...
DROP TABLE IF EXISTS mimic_etl.d_labitems_to_concept;
CREATE TABLE mimic_etl.d_labitems_to_concept AS
WITH
counts AS
(
    SELECT
    ...
    FROM
        mimic_etl.lk_meas_labevents_mapped
    GROUP BY itemid
)
SELECT
    -- itemid
...
FROM
    mimic_etl.lk_meas_d_labitems_concept src
LEFT JOIN
    counts 
        USING (itemid)
ORDER BY
...
DROP TABLE IF EXISTS mimic_etl.d_micro_to_concept;
CREATE TABLE mimic_etl.d_micro_to_concept AS
WITH
counts AS
(
    SELECT
    ...
    FROM
        mimic_etl.lk_specimen_mapped
    GROUP BY itemid
    UNION ALL -- more unions
    SELECT
        test_itemid AS itemid, COUNT(*) AS row_count
    FROM
        mimic_etl.lk_meas_organism_mapped
    GROUP BY itemid
    UNION ALL -- more unions
    SELECT
...
SELECT
    ...
    counts.row_count                AS row_count
FROM
    mimic_etl.lk_d_micro_concept src
LEFT JOIN
    counts 
        USING (itemid)
LEFT JOIN
