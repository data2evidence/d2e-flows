...
DROP TABLE IF EXISTS mimic_etl.cdm_dose_era;
CREATE TABLE mimic_etl.cdm_dose_era
(
...
-- -------------------------------------------------------------------
DROP TABLE IF EXISTS mimic_etl.tmp_drugIngredientExp;
CREATE TABLE mimic_etl.tmp_drugIngredientExp AS
...
FROM mimic_etl.cdm_drug_exposure de
         INNER JOIN mimic_etl.voc_drug_strength ds
                    ON de.drug_concept_id = ds.drug_concept_id
         INNER JOIN mimic_etl.voc_concept_ancestor ca
                    ON de.drug_concept_id = ca.descendant_concept_id
                        AND ds.ingredient_concept_id = ca.ancestor_concept_id
         LEFT JOIN mimic_etl.voc_concept c
...
DROP TABLE IF EXISTS mimic_etl.tmp_drugWithDose;
CREATE TABLE mimic_etl.tmp_drugWithDose AS
...
FROM mimic_etl.tmp_drugIngredientExp
;
...
DROP TABLE IF EXISTS mimic_etl.tmp_cteDoseTarget;
CREATE TABLE mimic_etl.tmp_cteDoseTarget AS
SELECT dwd.drug_exposure_id                                         AS drug_exposure_id,
       ...
       COALESCE(drug_exposure_end_date,
           ...
           -- return drug_exposure_end_date, otherwise go to next case
                NULLIF(drug_exposure_start_date + INTERVAL '1 DAY' * days_supply * (COALESCE(refills, 0) + 1), drug_exposure_start_date),
...
FROM mimic_etl.tmp_drugWithDose dwd
WHERE dose_value <> -1
;
...
DROP TABLE IF EXISTS mimic_etl.tmp_cteDoseEndDates_rawdata;
CREATE TABLE mimic_etl.tmp_cteDoseEndDates_rawdata AS
...
FROM mimic_etl.tmp_cteDoseTarget
UNION ALL
...
DROP TABLE IF EXISTS mimic_etl.tmp_cteDoseEndDates_e;
CREATE TABLE mimic_etl.tmp_cteDoseEndDates_e AS
SELECT person_id                                                     AS person_id,
       drug_concept_id                                               AS drug_concept_id,
       unit_concept_id                                               AS unit_concept_id,
       dose_value                                                    AS dose_value,
       ...
       MAX(start_ordinal) OVER (
           PARTITION BY person_id, drug_concept_id, unit_concept_id, CAST(dose_value AS bigint) -- double-check if it is a valid cast
...
FROM mimic_etl.tmp_cteDoseEndDates_rawdata
;
...
DROP TABLE IF EXISTS mimic_etl.tmp_cteDoseEndDates;
CREATE TABLE mimic_etl.tmp_cteDoseEndDates AS
SELECT person_id                             AS person_id,
       drug_concept_id                       AS drug_concept_id,
       unit_concept_id                       AS unit_concept_id,
       dose_value                            AS dose_value,
       event_date - INTERVAL '30 DAYS' AS end_date -- unpad the end date
FROM mimic_etl.tmp_cteDoseEndDates_e
...
DROP TABLE IF EXISTS mimic_etl.tmp_cteDoseFinalEnds;
CREATE TABLE mimic_etl.tmp_cteDoseFinalEnds AS
...
FROM mimic_etl.tmp_cteDoseTarget dt
         INNER JOIN mimic_etl.tmp_cteDoseEndDates e
...
INSERT INTO mimic_etl.cdm_dose_era
SELECT nextval('id_sequence') AS dose_era_id, t.*
FROM (SELECT person_id                     AS person_id,
      ...
      FROM mimic_etl.tmp_cteDoseFinalEnds
      GROUP BY person_id,
               drug_concept_id,
               unit_concept_id,
               dose_value,
